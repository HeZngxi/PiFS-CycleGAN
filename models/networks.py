...
# # 定义生成器示例，用于CT图像到MRI图像的转换。创建之后需要使用init_net进行网络权重初始化，才能用于具体任务
...
def define_D(input_nc, ndf, netD, n_layers_D=4, norm="batch", init_type="normal", init_gain=0.02):
    """
    创建一个判别器。

    此函数现在支持创建 'multiscale' 判别器。

    参数:
        ... (参数说明与之前相同) ...
        netD (str): 判别器架构的名称: 'basic' | 'n_layers' | 'pixel' | 'multiscale'
        n_layers_D (int): 当 netD 为 'n_layers' 或 'multiscale' 时，指定判别器的层数。
    """
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    if netD == 'basic':
        ...
        print(f"Using MultiScaleDiscriminator with {n_layers_D} layers for each sub-discriminator.")
        net = MultiScaleDiscriminator(input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_layer)
...
class MINDLoss(nn.Module):
    def __init__(self, patch_size=7, non_local_size=9, gaussian_sigma=2.0):
        super(MINDLoss, self).__init__()
        self.patch_size = patch_size
        self.non_local_size = non_local_size
        self.gaussian_sigma = gaussian_sigma
        
        # 预计算高斯核，并调整为卷积权重的形状 (Out, In, H, W) -> (1, 1, P, P)
    ...
    def _create_gaussian_kernel(self):
        ...
        # 调整形状以适配 F.conv2d: (out_channels, in_channels, kH, kW)
        return kernel.view(1, 1, size, size)

    def _calculate_mind_for_image(self, image):
        # 如果图像是多通道的，转换为单通道
        if image.shape[1] > 1:
            image = torch.mean(image, dim=1, keepdim=True)
        
        # 图像范围为[-1,1]，转换到[0,1]范围以适应MIND计算
        image = (image + 1.0) / 2.0
        ...
        # 计算特征维度
        # 论文定义 R_nl 为非局部区域，MIND 向量长度为 |R_nl| (通常排除中心点)
        feature_dim = self.non_local_size * self.non_local_size - 1
        mind_features = []

        search_radius = self.non_local_size // 2
        
        # 填充图像以便处理边界 (使用 reflect 模式)
        # 这里的 padding 是为了 shift 操作，不是为了卷积
        ...
        # --- 步骤 1: 计算所有位移的 patch 距离 Dp (公式 1 & 4) ---
        for dy in range(-search_radius, search_radius + 1):
            for dx in range(-search_radius, search_radius + 1):
                ...
                # Neighbor: 从 pad_len + dy 开始
                # 切片范围: [pad_len + dy : pad_len + dy + height]
                y_start = pad_len + dy
                x_start = pad_len + dx
                ...
                # 2. 使用高斯核进行卷积聚合 (公式 4: C * diff_sq)
                # padding=self.patch_size//2 保持尺寸不变 (same padding)
        ...
        for dy in range(-1, 2):
            for dx in range(-1, 2):
                if dy == 0 and dx == 0:
                    continue
                # 检查是否在我们计算的范围内 (通常 non_local_size >= 3，所以一定在)
                if (dy, dx) in dp_maps:
        ...
        mind_final = torch.exp(-mind_features / vx) # vx 会自动广播
        
        return mind_final

    def forward(self, input_image, target_image):
        # 确保输入是 (N, C, H, W)
        mind_in = self._calculate_mind_for_image(input_image)
        mind_tar = self._calculate_mind_for_image(target_image)
        
        # 计算每个样本的损失
        abs_diff = torch.abs(mind_in - mind_tar)
        # 对特征维度和空间维度求平均，得到每个样本的损失值
        loss_per_sample = torch.mean(abs_diff, dim=(1, 2, 3))
        
        # 输出每个样本的损失值（仅在训练模式下）
        if self.training:
            print("MIND Loss per image in batch:")
            for i, loss_val in enumerate(loss_per_sample):
                print(f"  Image {i}: {loss_val.item():.6f}")
            print(f"  Batch average: {torch.mean(loss_per_sample).item():.6f}")
...
# 添加MultiMINDLoss类，实现多尺度MIND损失计算
class MultiMINDLoss(nn.Module):
    """
    多尺度MIND损失类，用于平衡物理模拟EUS图像和模型推理EUS图像在纹理和结构上的差异
    在低分辨率尺度上关注结构特征，在高分辨率尺度上关注纹理细节
    """
    def __init__(self, scales=[1.0, 0.25, 0.0625], weights=[0.1, 0.35, 0.55], patch_sizes=[13,13,13], non_local_sizes=[15,15,15], gaussian_sigma=2.0, verbose=False, use_gradient_weight=False):
        """
        初始化多尺度MIND损失模块

        参数:
        - scales: 尺度列表，例如 [1.0, 0.5, 0.25] 表示原始、1/2、1/4
        - weights: 每个尺度的权重，例如 [0.6, 0.3, 0.1]，低尺度（结构）权重更高
        - patch_sizes: 每个尺度对应的patch_size，例如 [7, 5, 3]
        - non_local_sizes: 每个尺度对应的non_local_size，例如 [9, 7, 5]
        - gaussian_sigma: 高斯核的标准差
        - verbose: 是否输出调试信息
        - use_gradient_weight: 是否使用梯度权重
        """
        super(MultiMINDLoss, self).__init__()
        self.scales = scales
        self.weights = weights
        self.patch_sizes = patch_sizes
        self.non_local_sizes = non_local_sizes
        self.gaussian_sigma = gaussian_sigma
        self.verbose = verbose
        self.use_gradient_weight = use_gradient_weight

        # 验证输入参数的长度是否一致
        assert len(scales) == len(weights) == len(patch_sizes) == len(non_local_sizes), \
            "scales, weights, patch_sizes, and non_local_sizes must have the same length"
    
    def forward(self, label_image, eus_image, label_for_weight=None):
        """
        计算多尺度MIND损失
        ...
        - label_for_weight: 用于计算梯度权重的标签图像，形状为 (B, C, H, W)
        """
        total_loss = 0.0
        device = label_image.device

        # 导入MultiMIND模块
        from .MultiMIND import compute_multiscale_mind

        # 调用MultiMIND.py中的多尺度计算函数
        loss = compute_multiscale_mind(
            label_image, 
            eus_image, 
            self.scales, 
            self.weights, 
            self.patch_sizes, 
            self.non_local_sizes, 
            self.gaussian_sigma,
            self.verbose,
            self.use_gradient_weight,
            label_for_weight  # 使用提供的label_for_weight作为计算梯度权重的参考
        )
